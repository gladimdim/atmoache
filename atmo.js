...
var fnGetForecastWithString = function(sURL) {
	...
	if (xml.readyState === 4 && xml.status === 200) {
		...
		var color = colorForDiff(aDiffs[i]);		
		$("#pressure").append("<div class='div-diff' style='background-color: rgb( " + color + ");'>" + currentDate.toDateString() + "</div>");
	    }
	}
    });
...
var fnCalculateDifferences = function(aPressures) {
	...
	aReturn.push(Math.abs(a - b).toFixed(0));
    }
    console.log(aReturn);
    return aReturn;
};
